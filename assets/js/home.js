...
function validation() {
  ...
  if (isNaN(tMatch) || isNaN(tWr) || isNaN(wrReq)) {
    text = `Isi Semua Kalo Mau Tau Hasilnya Mah Atuh..`;
    display(text);
    ...
    text = `Oke Hasilnya Kamu Perlu <b>0</b> win tanpa lose untuk mendapatkan win rate <b>${wrReq}%</b>`;
    display(text);
    ...
    text = `Jangan Aneh Aneh Yaaah`;
    display(text);
  } else if (resultNum >= 100000) {
    text = `<b>WKWKWWKWK GABAKAL BISA!</b> <br> Kamu perlu lebih dari <b>100.000</b> win tanpa lose untuk mendapatkan win rate <b>${wrReq}%</b>`;
    display(text);
  } else {
